...
const SavedItems = () => {
    const [activeTab, setActiveTab] = useState<'wishlist' | 'searches'>('wishlist');
    ...
    const handleRunSearch = (search: SavedSearch) => {
        ...
        if (search.filters.distance) params.set('distance', search.filters.distance.toString());
        if (search.filters.condition) params.set('condition', search.filters.condition);

        router.push(`/tools/find-products/search-results?q=${search.query}&${params.toString()}`);
    };
        ...
        <div className="space-y-4">
            ...
            <div className="flex space-x-1 bg-surface-secondary rounded-lg p-1">
                <button
                    ...
                    className={`flex-1 flex items-center justify-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${activeTab === 'wishlist' ? 'bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'
                        }`}
                >
                    <Icon name="Heart" size={16} />
                    <span>Wishlist</span>
                    <span className={`px-2 py-1 rounded-full text-xs ${activeTab === 'wishlist' ? 'bg-primary text-white' : 'bg-border text-text-tertiary'
                        }`}>
                ...
                <button
                    ...
                    className={`flex-1 flex items-center justify-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${activeTab === 'searches' ? 'bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'
                        }`}
                >
                    <Icon name="Search" size={16} />
                    <span>Saved Searches</span>
                    <span className={`px-2 py-1 rounded-full text-xs ${activeTab === 'searches' ? 'bg-primary text-white' : 'bg-border text-text-tertiary'
                        }`}>
                    ...
                    {wishlistItems.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {wishlistItems.map((item) => (
                                <div
                                    key={item.id}
                                    className={`bg-surface border border-border rounded-lg overflow-hidden hover:shadow-elevation-2 transition-shadow duration-200 ${!item.isAvailable ? 'opacity-75' : ''
                                        }`}
                                >
                                    ...
                                    <div className="p-4">
                                        ...
                                        <div className="flex space-x-2">
                                            {item.isAvailable ? (
                                                <>
                                                    <button
                                                        onClick={() => router.push('/tools/find-products/product-detail')}
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="text-text-secondary mb-6">
                                Save items you're interested in to keep track of them and get notified of price changes.
                            </p>
                            <button
                ...
                <div className="space-y-4">
                    {savedSearches.length > 0 ? (
                                    ...
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex-1">
                                            ...
                                            <div className="text-xs text-text-tertiary">
                                                Created {formatDate(search.createdDate)} • Last checked {formatDate(search.lastChecked)}
                                            </div>
                                        </div>
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="text-text-secondary mb-6">
                                Save your searches to get notified when new matching items are listed.
                            </p>
                            <button
