...
export default function Receipts({ invoices, loading, refetchInvoices }: ReceiptProps) {
    ...
    const processInvoiceItemForDisplay = (item: ApiReceiptItem): ProcessedReceiptItem => {
    ...
    if (loading || isLoading) {
        return (
            <Card className="border-0 min-h-[90vh] shadow-none">
                ...
                <CardContent>
                    <div className="text-center py-8 text-gray-500">Loading receipt records...</div>
                </CardContent>
            </Card>
        );
    }
    ...
    if (!invoices || invoices.length === 0) {
        return (
            <Card className="border-0 min-h-[90vh] shadow-none">
                ...
                <CardContent>
                    <div className="text-center py-8 text-gray-500">No receipt records yet.</div>
                </CardContent>
            </Card>
        );
    }
            ...
            <CardContent className=' px-3 sm:px-6'>
                ...
                <div className="space-y-4">
                    {filteredInvoices.length === 0 ? (
                        <div className="text-center py-8 text-gray-500">No matching receipt records found.</div>
                    ) : (
                        filteredInvoices.map((invoice) => (
                            <div key={invoice.originalId} className="flex flex-col sm:flex-row items-start sm:items-center justify-start sm:justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                                ...
                                <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-2 sm:mt-0 sm:ml-auto">
                                    ...
                                    <div className="flex items-center gap-1 ml-auto sm:ml-4">
                                        ...
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            onClick={() => handleDeleteReceipt(invoice.originalId)}
