...
const NotificationToast = ({
  ...
  return (
    <div
      className={`fixed top-4 right-4 z-50 p-4 rounded-xl border shadow-2xl max-w-sm backdrop-blur-sm ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 shadow-sm">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-800">Authentication Status</p>
              <p className="text-xs text-amber-700">
                Token: {getAuthToken() ? "Connected" : "Not found"}
                {getAuthToken() && ` (${getAuthToken().substring(0, 20)}...)`}
              </p>
            </div>
          </div>
        </div>
