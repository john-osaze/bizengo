    ...
    const renderIcon = (): React.ReactNode => {
        ...
        return React.isValidElement(icon)
            ? React.cloneElement(icon as React.ReactElement<any>, {
                ...(icon.props || {}),
                className: `${(icon.props && typeof icon.props === 'object' && icon.props !== null && 'className' in icon.props ? (icon.props.className as string) : '')} ${children ? (iconPosition === 'left' ? 'mr-2' : 'ml-2') : ''} h-5 w-5`.trim()
            })
            : null;
    };
